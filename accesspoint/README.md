...
- **3× Seeed Studio XIAO ESP32-C6** — the ESP32-C6 has the WiFi MAC/PHY support required for both the FTM initiator and responder roles.
...
|---|---|---|---|
| `ftm_responder_a` | `demos/ftm_responder.cpp` | Anchor A. Brings up a SoftAP (`FTM_Anchor_A`, channel 1) with the 802.11mc FTM responder bit set. Idle otherwise — the WiFi driver answers FTM frames in the background. | `/dev/ttyACM0` |
| `ftm_responder_b` | `demos/ftm_responder.cpp` | Anchor B. Same firmware as A, but the SSID (`FTM_Anchor_B`) and channel are injected via build flags. | `/dev/ttyACM1` |
| `ftm_initiator` | `demos/ftm_initiator.cpp` | Scans for every AP whose SSID starts with `FTM_Anchor_`, records each one's BSSID + channel, then loops over that list running an unassociated FTM burst against each anchor and printing its distance estimate. | `/dev/ttyACM2` |
